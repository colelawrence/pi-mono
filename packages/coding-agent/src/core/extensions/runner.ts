...
const RESERVED_KEYBINDINGS_FOR_EXTENSION_CONFLICTS = [
	...
	"app.tools.expand",
	"app.thinking.toggle",
	"app.customMessages.toggleHidden",
	"app.editor.external",
...
export class ExtensionRunner {
	...
	bindCore(
		...
		this.runtime.getHostCapabilities = actions.getHostCapabilities;
	...
	createContext(): ExtensionContext {
		...
		const ctx: ExtensionContext = {
			...
			get hostCapabilities() {
				runner.assertActive();
				return runner.runtime.getHostCapabilities();
			},
			get model() {
				runner.assertActive();
				return getModel();
	...
	createCommandContext(): ExtensionCommandContext {
		...
		if (this._epiRuntime) {
			(context as any)[EPI_RUNTIME_SYMBOL] = this._epiRuntime;
		}
		return context;
	}
