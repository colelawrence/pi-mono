...
export class AgentSession {
	...
	private _installAgentToolHooks(): void {
		this.agent.beforeToolCall = async ({ toolCall, args }) => {
			...
			await this._runtime.drainEventQueue();
	...
	private async _handleRetryableError(message: AssistantMessage): Promise<boolean> {
		...
		// Register the controller before emitting auto_retry_start so an immediate
		// user abort can cancel the sleeping backoff reliably.
		const retryAbortController = new AbortController();
		this._runtime.setRetryAbortController(retryAbortController);
