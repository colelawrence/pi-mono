...
describe("AgentSession retry", () => {
	...
	it("abortRetry cancels a sleeping retry backoff before it schedules continuation", async () => {
		const created = createSession({ failCount: 99, maxRetries: 3, baseDelayMs: 100 });
		const events: string[] = [];
		...
		created.session.subscribe((event) => {
			if (event.type === "auto_retry_start") {
				...
				if (!abortScheduled) {
					abortScheduled = true;
					setTimeout(() => created.session.abortRetry(), 0);
				}
			}
			if (event.type === "auto_retry_end") events.push(`end:success=${event.success}`);
		});
		...
		expect(events).toEqual(["start:1", "end:success=false"]);
		expect(created.session.isRetrying).toBe(false);
	});
