...
## Sync 2026-05-26
- merged: `upstream/main @ 7c2775f6`
- branch: `sync/upstream-2026-05-26`
- conflicts:
  - `.husky/pre-commit` — kept staged-file restage behavior and added upstream lockfile guard/check flow
  - `package.json`, `packages/coding-agent/package.json`, `package-lock.json` — adopted upstream `0.75.5` / `@earendil-works/*` package state and pinned dependency policy while preserving downstream-required `effect`, `@opentelemetry/api`, and `@sinclair/typebox`
  - `packages/ai/src/models.generated.ts` — resolved by regeneration from the merged branch state; `packages/ai/src/image-models.generated.ts` was also regenerated by the upstream build scripts
  - `packages/coding-agent/src/core/agent-session.ts` — kept upstream provider/auth/thinking/session-resource flow and reapplied downstream `SessionRuntime`, lifecycle fence, epi runtime bridge, host capabilities, and `epi_user_turn_ready` hooks using `.ts` import style
  - `packages/coding-agent/src/core/extensions/loader.ts` — kept upstream `@earendil-works` loader/source-info/path normalization and re-applied downstream host capabilities plus layered-extension descriptors
  - `packages/coding-agent/src/core/tools/bash.ts`, `packages/coding-agent/src/utils/child-process.ts` — kept upstream async cwd check / output accumulator / Windows spawn support and reapplied downstream wait metadata + bash telemetry details
  - `packages/coding-agent/src/modes/interactive/interactive-mode.ts` — kept upstream async external editor path and retained downstream hidden custom-message toggle
  - `packages/coding-agent/test/extensions-runner.test.ts`, `packages/coding-agent/test/trigger-compact-extension.test.ts`, `packages/coding-agent/test/agent-session-runtime-invariants.test.ts`, `packages/coding-agent/test/bash-telemetry-details.test.ts`, `packages/coding-agent/test/host-capabilities.test.ts`, `packages/coding-agent/test/tool-call-security.test.ts`, `packages/coding-agent/test/user-turn-ready.test.ts`, `packages/agent/test/structured-tool-error-result.test.ts` — updated downstream tests to upstream `.ts` import and `@earendil-works/*` package conventions while preserving carry coverage
  - `packages/tui/src/tui.ts` — kept upstream Kitty image cleanup helpers and retained downstream `truncateToWidth` import for loud-but-non-fatal render recovery
  - `packages/coding-agent/CHANGELOG.md`, `packages/tui/CHANGELOG.md` — combined downstream unreleased entries with upstream release notes
- verification:
  - [x] `npm run check`
  - [x] `cd packages/tui && node --test --import tsx test/tui-render.test.ts`
  - [x] `cd packages/ai && npm run build && cd ../agent && npm run build`
  - [x] `cd packages/tui && npm run build`
  - [x] `cd packages/coding-agent && npx tsx ../../node_modules/vitest/dist/cli.js --run test/host-capabilities.test.ts test/trigger-compact-extension.test.ts test/user-turn-ready.test.ts test/agent-session-runtime-invariants.test.ts test/suite/agent-session-runtime.test.ts`
  - [x] `./install-epi.sh`
- notes:
  - current carry patches still expected: `packages/tui/src/tui.ts` for loud-but-non-fatal render failures/overwide lines, `packages/coding-agent/src/core/agent-session.ts` / `session-runtime.ts` / `extensions/runner.ts` / `user-turn-ready.ts` for epi runtime + lifecycle fence + turn-ready integration, `packages/coding-agent/src/core/extensions/loader.ts` / `host-capabilities.ts` for downstream host-capability + layered-extension support, `packages/coding-agent/src/modes/interactive/interactive-mode.ts` for hidden custom-message visibility, `packages/coding-agent/src/core/tools/bash.ts` / `utils/child-process.ts` for bash telemetry details, and `packages/agent/src/proxy.ts` for structural proxy typing
  - `packages/ai/src/models.generated.ts` conflicted and was regenerated; `packages/ai/src/image-models.generated.ts` was also regenerated by `npm run build` / `./install-epi.sh`, so this sync keeps generated catalog churn in the merge rather than hand-resolving it
  - this sync was prepared in a temporary worktree because the primary checkout had pre-existing local dirt in `packages/coding-agent/CHANGELOG.md`, `packages/coding-agent/src/core/agent-session.ts`, and `packages/coding-agent/test/agent-session-auto-compaction-queue.test.ts`
- reflection:
  - surprisingly easy: once upstream package entrypoints were built locally, the runtime/carry tests passed with only import-namespace and package-entrypoint adjustments
  - repeated friction: the upstream package rename to `@earendil-works/*`, exact dependency pinning, shrinkwrap checking, and no-relative-`.js` import rule turn old downstream test/carry files into check failures even after conflicts are resolved
  - next carry to reduce: move the bash telemetry metadata and epi runtime/lifecycle carries behind smaller seams before the next large upstream sync
  - process update: for post-0.75 syncs, resolve downstream imports to `.ts` and `@earendil-works/*` during conflict resolution, run `node scripts/generate-coding-agent-shrinkwrap.mjs` after dependency changes, and build `packages/ai`, `packages/agent`, and `packages/tui` before coding-agent vitest when using a fresh temporary worktree
